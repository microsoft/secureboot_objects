...
def get_certificate_info(certificate_der: bytes) -> dict:
    ...
    # This avoids issues with signed/unsigned integer interpretation
    serial_number = cert.serial_number
    # Handle negative serial numbers (non-compliant certificates with high bit set)
    if serial_number < 0:
        # Convert to unsigned by adding 2^(bit_length)
        bit_length = serial_number.bit_length() + 1
        serial_number = serial_number + (1 << bit_length)
...
def process_directory(directory_path: str, output_json: str) -> None:
    ...
    for bin_file in bin_files:
        try:
            ...
            results[key] = {
                "KEKUpdate": str(relative_path).replace(os.sep, "\\"),
            ...
            logging.warning(f"Failed to process {bin_file.relative_to(directory)}: {e}")
            continue
